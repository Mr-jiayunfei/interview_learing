...
有三种，普通构造函数，拷贝构造函数，移动构造函数

当使用有参或则无参的时候，定义一个对象的时候，调用构造函数

当使用一个对象初始化另一个对象的时候调用拷贝构造函数

当使用一个对象初始化另一个对象，并且这个对象是右值的时候，调用移动构造函数，如std::move
...
如果没有使用初始化表，则该类的成员变量在构造函数体执行之前先定义并且执行默认初始化，随后在构造函数体内进行赋值操作，但是有些变量必须在定义的时候直接初始化，不能先定义后赋值，如const变量、引用类型的变量、成员属于某种类类型并且该类没有提供默认构造函数时等，这几种情况必须使用初始化表。
...
如果没有将基类的析构函数设计为虚函数，当delete一个指向子类对象的基类指针时，调用的只是基类的析构函数，不会调用子类的析构函数，可能会形成内存泄漏。

当将基类的析构函数设计为虚函数的时候，子类的析构函数会自动成为虚析构函数，当delete一个指向子类对象的基类指针时，会调用子类的析构函数，进而调用基类的析构函数，不会形成内存泄漏的危险。
...
智能指针的出现主要是为了更安全的使用动态内存

当我们使用new为对象分配空间并返回一个指向该对象的指针时，这个指针就是普通指针，内存空间使用完毕后，我们需要手动调用delete销毁该对象，并释放内存，但是很容易出问题，比如：1 忘记释放内存，会产生内存泄漏   2已经释放完内存，程序后面仍继续使用这个指针 3 程序产生异常，无法执行到delete的代码等等，智能指针可以解决这些问题。

智能指针是一个模板类，当离开作用域时，可以调用其析构函数自动释放内存资源。

# 智能指针有几种？

shared_ptr,unique_ptr,weakPtr

shared_ptr允许多个指针指向同一个对象，unique_ptr则独占所指向的对象，weak_ptr是一种弱引用，指向shared_ptr所管理的对象。

```c++
//make_shared会在动态内存中分配一个对象并初始化它
...
weak_ptr的实现原理是在其内部维护了一个weak_count计数器，同时shared_ptr和weak_ptr拥有相同的基类，这个基类中有两个数据成员，_Uses和_Weaks，
...
shared_ptr适用于多个对象共享相同的底层数据

unique_ptr适用于一个对象独享一个数据

weak_ptr可以解决悬空指针和shared_ptr的循环引用问题



# 为什么要有weak_ptr？

1. weak_ptr可以解决悬空指针的问题，仅通过使用原始指针，不知道所引用的数据是否已被释放，可以使用expired函数判断对象是否已经被销毁
2. weak_ptr可以解决shared_ptr的循环引用问题，
...
2 构造函数直接使用指针即可，不需要动态分配内存（构造函数的参数必须是new的指针）
...
虚函数是通过虚函数表和虚函数表指针实现的，

如果一个类内部有虚函数，那这个类声明的对象的首个地址就是虚函数表指针，这个指针指向虚函数表，

虚函数表是一块连续的内存，每个内存单元中记录了一个函数地址，我们可以通过类型数组的计算，得到函数的地址。

虚函数主要是为了实现多态，即子类对象的基类指针或引用调用的是子类的虚函数
...
采用这种机制的主要原因是提高内存系统性能，例如，假设计算机总是从内存中取8个字节，如果一个double数据的地址对齐成8的倍数，那么一个内存操作就可以读或者写，但是如果这个double数据的地址没有对齐，数据就可能被放在两个8字节块中，那么我们可能需要执行两次内存访问，才能读写完成。显然在这样的情况下，是低效的。所以需要字节对齐来提高内存系统性能。
